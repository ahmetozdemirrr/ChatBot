...
class ActionFindProductLocationMall(Action):


    def name(self) -> Text:

        return "action_find_product_location_mall"
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        product_weight = tracker.get_slot('product_weight')
        product_price = tracker.get_slot('product_price')
        product_warranty = tracker.get_slot('product_warranty')
        product_campaign = tracker.get_slot('product_campaign')
        ...
        if not campaign_ok:
            ...
            dispatcher.utter_message(response="utter_ask_mall_campaign")
        ...
        try:
            ...
            # Build the query
            query = {
                'aisles.shelves.shelves.product_category': product.capitalize() if product else None,
            }

            # Add optional criteria if provided
            if product_color:
                query['aisles.shelves.shelves.product.renk'] = product_color.lower()

            if product_size:
                query['aisles.shelves.shelves.product.boyut'] = product_size

            if product_weight:
                query['aisles.shelves.shelves.product.ağırlık'] = product_weight

            if product_price:
                query['aisles.shelves.shelves.product.fiyat'] = product_price

            if product_warranty:
                query['aisles.shelves.shelves.product.garanti'] = product_warranty

            query['aisles.shelves.shelves.product.kampanyali'] = product_campaign
            ...
            if not results:
                ...
                return [
                    ...
                    SlotSet("product_color", None),
                    SlotSet("product_size", None),
                    SlotSet("product_weight", None),
                    SlotSet("product_price", None),
                    SlotSet("product_warranty", None),
                    SlotSet("product_campaign", None),
            ...
            for result in results:
                ...
                if isinstance(product_details, list):
                    ...
                    for product in product_details:
                        ...
                        if not product_color or (product_category and product['renk'] == product_color.lower()):
                    ...
                    if not product_color or (product_category and product_details['renk'] == product_color.lower()):
